...
void DryBones::onCollide(Unit & collidingObject, int collidingObjectIndex) 
{
	...
	case PLAYER:
		...
		break;
	default:
		if ((mCurrentLocation.mX <= collidingObject.getLocation().mX || mCurrentLocation.mX + collidingObject.mDimensions.mX >= collidingObject.getLocation().mX + collidingObject.getDimensions().mX) && abs(collidingObject.getLocation().mY - mCurrentLocation.mY) < mDimensions.mY / 2)
		{
			changeDirection();
		}
		else
